...
def findAlternativeMove():
    ...
    if grid[(y-1)%height][x] != '#' and not enemyAtPos(x, y-1) and not enemyAtPos(x, y-2) and not enemyAtPos(x-1, y-1) and not enemyAtPos(x+1, y-1):
        return DIR['UP']
    if grid[y][(x+1)%width] != '#' and not enemyAtPos(x+1, y) and not enemyAtPos(x+2, y) and not enemyAtPos(x+1, y-1) and not enemyAtPos(x+1, y+1):
        return DIR['RIGHT']
    if grid[(y+1)%height][x] != '#' and not enemyAtPos(x, y+1) and not enemyAtPos(x, y+2) and not enemyAtPos(x-1, y+1) and not enemyAtPos(x+1, y+1):
        return DIR['DOWN']
    if grid[y][(x-1)%width] != '#' and not enemyAtPos(x-1, y) and not enemyAtPos(x-2, y) and not enemyAtPos(x-1, y-1) and not enemyAtPos(x-1, y+1):
